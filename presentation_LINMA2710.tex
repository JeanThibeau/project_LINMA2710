...
\begin{itemize}
  \item Local compute time: the triple loop that forms the partial product.
  \item Communication time: the \texttt{MPI\_Allreduce} that sums partial results.
  \item Reported fraction: $\alpha = T_{comm}/(T_{comm}+T_{comp})$.
\end{itemize}

\textbf{Observed behavior:}
\begin{itemize}
  \item For small matrices, communication becomes visible once the number of ranks grows.
  \item For large matrices, local computation dominates and the reduction cost is amortized.
  \item Example: for $512\times512$ on 8 ranks, the average communication fraction was about 15\% to 23\%.
\end{itemize}

\textbf{Interpretation:}
The algorithm scales well when the matrix is large enough, but MPI collectives dominate for small matrices or many ranks.
\end{frame}
...
\textbf{Measured total times and speedups:}
\scriptsize
\begin{tabular}{rcccc}
\toprule
Size & $T_1$ (ms) & $S(2)$ & $S(4)$ & $S(8)$ \\
\midrule
64  & 1.129 & 1.69x & 3.73x & 4.24x \\
128 & 8.883 & 1.85x & 4.30x & 5.91x \\
256 & 69.729 & 1.88x & 4.67x & 6.77x \\
512 & 553.988 & 2.45x & 4.81x & 7.40x \\
\bottomrule
\end{tabular}

\vspace{0.15cm}
\normalsize
\textbf{Conclusion:} the measured speedup gets closer to ideal as the matrix size increases. The best result is about $7.4\times$ on 8 ranks for $512\times512$.
\end{frame}
...
\begin{itemize}
  \item For small matrices or many ranks, MPI collectives dominate the runtime.
  \item For large matrices, the local multiply dominates and the collective cost is amortized.
\end{itemize}

\textbf{Expected speedup}
\begin{itemize}
  \item Ideal upper bound is close to $p$.
  \item In practice, your measured speedup reaches about $7.4\times$ on 8 ranks for $512\times512$.
\end{itemize}
...
\begin{itemize}
  ...
  \item Trade-off: column splitting is natural for matrix operations, while gradient sync is better when repeated local updates can hide communication.
\end{itemize}
\end{frame}
